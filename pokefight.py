...
def fight_pokemon():
    ...
    time.sleep(random_delay(0.1, 0.5))  # Wait a moment to ensure the fight menu appears
    pyautogui.press('e')  # Select the first move
    time.sleep(random_delay(0.5, 1.5))  # Wait a moment to execute the move
    ...
    while True:
        ...
        if found_pokemon:
            while pokemon_still_alive():
                fight_pokemon()
                time.sleep(random_delay(0.5, 1.5))  # Wait between attacks to ensure move execution
            print(f"Defeated {found_pokemon}!")
            time.sleep(random_delay(1.5, 2.5))  # Wait a moment before starting to run again
